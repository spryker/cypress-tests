# Migration progress

Generated by `render.py` — do not edit. Run `python3 build.py && python3 render.py`.

| Matrix | Domains | Scenarios | MIGRATE | ported | dropped | review | blocked | done % |
|---|--:|--:|--:|--:|--:|--:|--:|--:|
| cypress-api-to-codeception | 2 | 3 | 1 | 0 | 0 | 2 | 0 | 0% |

## cypress-api-to-codeception by domain

| Domain | Scenarios | MIGRATE | ported | dropped | review | blocked | batches |
|---|--:|--:|--:|--:|--:|--:|---|
| [content](by-domain/cypress-api-to-codeception/content.md) | 1 | 0 | 0 | 0 | 1 | 0 | 1 |
| [platform](by-domain/cypress-api-to-codeception/platform.md) | 2 | 1 | 0 | 0 | 1 | 0 | 1 |

## Review — unresolved judgement calls

| Domain | Scenario | Recommended |
|---|---|---|
| content | should be able to see the cms page for new store | keep in Cypress until store + CMS page can be seeded as dynamic fixtures, then migrate |
| platform | GLUE endpoint should return 200 | drop unless the smoke lane's fail-fast gating is worth keeping the duplicate |
