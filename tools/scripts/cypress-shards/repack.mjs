...
function parsePaste(text) {
  ...
  const cleanLine = (l) => l.replace(/^[─-╿\s]+/, '').replace(/[─-╿\s]+$/, '');
...
function main() {
  ...
  const manifest = Object.fromEntries(packed.map((s, i) => [String(i + 1), s.specs.sort()]));

  console.error('\nShard plan:');
  for (let i = 0; i < numShards; i++) {
    console.error(`  ${i + 1}: ${packed[i].specs.length} specs, ${formatDuration(packed[i].total)}`);
  }
