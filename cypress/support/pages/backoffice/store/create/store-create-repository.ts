...
export class StoreCreateRepository {
  ...
  getAvailableLocaleInput = (locale: string): Cypress.Chainable =>
    cy.get(`#available-locale-table_wrapper [value="${locale}"]`, { timeout: 10000 });
  getAvailableCurrencyInput = (currency: string): Cypress.Chainable =>
    cy.get(`#available-currency-table [value="${currency}"]`, { timeout: 10000 });
  ...
  getAvailableCountryInput = (country: string): Cypress.Chainable =>
    cy.get(`#available-country-table [value="${country}"]`, { timeout: 10000 });
  getSaveButton = (): Cypress.Chainable => cy.get('[type="submit"]');

  getStoreContextTabButton = (): Cypress.Chainable =>
    cy.get('[data-tab-content-id="tab-content-store_context"]', { timeout: 15000 });
  getAddStoreContextButton = (): Cypress.Chainable =>
    cy.get('#tab-content-store_context .btn.add-store-context', { timeout: 15000 });
  getTimezoneSelector = (): Cypress.Chainable =>
    cy.get('#store_applicationContextCollection_applicationContexts_1_timezone', { timeout: 25000 });
}
