...
export class StoreCreateRepository {
  ...
  getLocaleSearchInput = (): Cypress.Chainable => cy.get('#available-locale-table_wrapper input[data-qa="table-search"]');
  getCurrencySearchInput = (): Cypress.Chainable => cy.get('input[data-qa="table-search"][aria-controls="available-currency-table"]');
  getAvailableLocaleInput = (locale: string): Cypress.Chainable => cy.get(`#available-locale-table_wrapper [value="${locale}"]`);
  getAvailableCurrencyInput = (currency: string): Cypress.Chainable => cy.get(`#available-currency-table [value="${currency}"]`);
  getDisplayRegionsTab = (): Cypress.Chainable => cy.get('[data-tab-content-id="tab-content-country_store_relation"]');
  getCountrySearchInput = (): Cypress.Chainable => cy.get('input[data-qa="table-search"][aria-controls="available-country-table"]');
  getAvailableCountryInput = (country: string): Cypress.Chainable => cy.get(`#available-country-table [value="${country}"]`);
  ...
  getTimezoneSelector = (): Cypress.Chainable => cy.get('#store_applicationContextCollection_applicationContexts_1_timezone');
}
