...
(['suite'].includes(Cypress.env('repositoryId')) ? describe : describe.skip)(
  ...
  it ('guest should not see the ssp asset selector', () => {
    catalogPage.visit();
  ...
  it('should view the catalog page with ssp asset spare parts products', () => {
    ...
    catalogPage.getProductItemBlocks().first().should('contain', dynamicFixtures.productSpareParts.localized_attributes[0].name);
  ...
  it('should view the catalog page with ssp asset service products', () => {
    ...
    catalogPage.getProductItemBlocks().should('have.length', 1);
    catalogPage.getProductItemBlocks().first().should('contain', dynamicFixtures.productService.localized_attributes[0].name);
    catalogPage.getSspAssetNameBlock().should('contain', dynamicFixtures.sspAsset.name);
  });
});
