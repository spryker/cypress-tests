...
describe(
  ...
  (): void => {
    ...
    it(
      'Quick Order page opens (HTTP 200) and shows the "Add to cart from image" section',
      { tags: ['@demo-smoke'] },
      (): void => {
    ...
    it(
      'image-to-cart control shows an image-only file input, a browse-file label and an enabled Upload button',
      { tags: ['@demo-smoke'] },
      (): void => {
    ...
    it(
      'attaching an image file shows it as attached: the file name replaces the placeholder and the browse label releases its input binding',
      { tags: ['@demo-smoke'] },
      (): void => {
        quickOrderImageToCartPage.visitQuickOrder();
    ...
    it(
      'submitting the attached image issues a multipart image-to-cart POST and the FE handles the no-provider response gracefully',
      { tags: ['@demo-smoke'] },
      (): void => {
        quickOrderImageToCartPage.visitQuickOrder();
        quickOrderImageToCartPage.attachImage(staticFixtures.imageFilePath);
