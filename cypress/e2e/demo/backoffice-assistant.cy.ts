...
describe(
  ...
  (): void => {
    ...
    it(
      'assistant launcher and its chat dialog panel are present on the Back Office dashboard (page loads HTTP 200)',
      { tags: ['@demo-smoke'] },
      (): void => {
    ...
    it(
      'sending a message issues the assistant prompt POST with the message payload and recovers gracefully when the provider fails',
      { tags: ['@demo-smoke'] },
      (): void => {
    ...
    it(
      'attaching a file shows it as an attachment chip and the sent prompt POST carries the attachment, recovering gracefully when the provider fails',
      { tags: ['@demo-smoke'] },
      (): void => {
        backofficeAssistantPage.interceptPromptWithFailure();
    ...
    it(
      'adding the page form as context shows a context chip and prefixes the sent prompt POST with the form context, recovering gracefully when the provider fails',
      { tags: ['@demo-smoke'] },
      (): void => {
        backofficeAssistantPage.interceptPromptWithFailure();
