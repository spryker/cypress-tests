...
describe(
  ...
  (): void => {
    ...
    it(
      'page opens (HTTP 200) with the section heading, all ten column headers and the five summary stats cards',
      { tags: ['@demo-smoke'] },
      (): void => {
        auditLogsPage.visitAuditLogs().its('response.statusCode').should('eq', 200);

        auditLogsPage.getSectionTitle().should('contain.text', auditLogsPage.getSectionTitleText());
    ...
    it(
      'initializes a live DataTable (HTTP 200 JSON with the recordsTotal/recordsFiltered/data shape) and loads the stats partial over AJAX, and marks Configuration/Status/Created At sortable and the rest non-sortable',
      { tags: ['@demo-smoke'] },
      (): void => {
        ...
        auditLogsPage.getStatsCards().should('contain.text', auditLogsPage.getTotalInteractionsCardText());
    ...
    describe('real AI audit-log data (full, requires provider token)', { tags: ['@demo-full'] }, (): void => {
      ...
      it(
        ...
        function (): void {
          ...
          auditLogsPage.fetchTableData().then((baseline): void => {
            ...
            auditLogsPage.fetchTableData().then((after): void => {
              ...
              const row = after.rows.slice(0, newRowCount)[0];
              expect(
              ...
              expect(status, 'status reflects a successful interaction').to.contain(
                auditLogsPage.getSuccessStatusText()
              );
