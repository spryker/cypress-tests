...
if ! docker-compose -f ../docker-compose.cypress-post-deploy-workflow.yml exec -T cypress-tests bash -c "ENV_REPOSITORY_ID=$ENV_REPOSITORY_ID npm run $NPM_COMMAND"; then
     aws s3 cp ../artifacts s3://$ROBOT_TESTS_ARTIFACTS_BUCKET/cypress-ui/$ENV_REPOSITORY_ID/post-deploy-se/$CODEBUILD_BUILD_ID/ \
                         --recursive \
                         --expires "$(date -d '+7 days' --utc +'%Y-%m-%dT%H:%M:%SZ')"

    echo "Error: Cypress tests failed. Artifacts are uploaded to s3://$ROBOT_TESTS_ARTIFACTS_BUCKET/cypress-ui/$ENV_REPOSITORY_ID/post-deploy-se/$CODEBUILD_BUILD_ID/"
    exit 1
fi
